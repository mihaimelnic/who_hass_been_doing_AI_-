...
OUTPUT_FILE = "top_ai_authors_with_papers.json"
TOTAL_PAPERS = 10000
PAPERS_PER_PAGE = 200

def fetch_top_ai_papers():
    base_url = "https://api.openalex.org/works"
    params = {
        "filter": "concepts.id:C154945302,title_and_abstract.search:artificial intelligence",
...
def process_authors(papers):
    ...
    for paper in papers:
        for authorship in paper.get("authorships", []):
            ...
            # Try to get institution and country from the authorship entry
            if author_stats[author_id]["institution"] == "Unknown":
                institutions = authorship.get("institutions", [])
                if institutions:
                    author_stats[author_id]["institution"] = institutions[0].get("display_name", "Unknown")
                    author_stats[author_id]["country"] = institutions[0].get("country_code", "Unknown")
...
def print_top_authors(authors, count=10):
    print("\n" + "="*80)
    print(f"TOP {count} AI RESEARCHERS")
    print("="*80)
    print("Search Criteria:")
    print("- Concept ID: C154945302 (Artificial Intelligence)")
    print("- Text: 'artificial intelligence' in title/abstract\n")
...
if __name__ == "__main__":
    print("Fetching top AI papers with combined filters...")
    papers = fetch_top_ai_papers()
    ...
    output = {
        "metadata": {
            ...
            "search_parameters": {
                "concept": "Artificial Intelligence (C154945302)",
                "text_search": "artificial intelligence in title/abstract",
                "sort": "cited_by_count:desc"
            }
        },
        "authors": authors
