...
OUTPUT_FILE = "top_rl_authors_with_papers.json"
TOTAL_PAPERS = 10000
PAPERS_PER_PAGE = 200
SEARCH_TERM = "reinforcement learning"

def fetch_top_rl_papers():
    """Fetch top cited RL papers using title/abstract search"""
...
def process_authors(papers):
    """Process authors with their top 3 papers and affiliations"""
    ...
    for paper in papers:
        for authorship in paper.get("authorships", []):
            ...
            # Capture institution and country if available and not already stored
    ...
    for author_id, data in author_stats.items():
        top_papers = sorted(data["all_papers"], key=lambda x: -x["citations"])[:3]
...
def print_top_authors(authors, count=10):
    ...
    print("\n" + "=" * 80)
    print(f"TOP {count} REINFORCEMENT LEARNING RESEARCHERS")
    print("=" * 80)
    print(f"Search Criteria: Title/abstract contains '{SEARCH_TERM}'\n")

    for rank, author in enumerate(authors[:count], 1):
        print(f"{rank}. {author['name']} ({author['institution']}, {author['country']})")
